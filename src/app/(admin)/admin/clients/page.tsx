...
export default async function AdminClientsPage() {
  ...
  const { data: clients } = await supabase
    .from("profiles")
    .select("id, full_name, company_name, phone, email, client_plans!client_plans_client_id_fkey(plan_name, status)")
    .eq("is_admin", false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <GradientButton href="/admin/clients/new">
      ...
      {clients && clients.length > 0 ? (
        ...
        <GlassCard>
          <p className="text-sm text-text-secondary">No clients yet — create the first one once a plan is sold.</p>
        </GlassCard>
      )}
    </div>
  );
